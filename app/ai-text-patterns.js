...
export function normalize(text) {
  return String(text)
    .normalize('NFC')
    .replace(/[‘’‚‛′]/g, "'")   // curly / prime single
    .replace(/[“”„‟″]/g, '"')   // curly / prime double
...
export const CATEGORIES = [
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    patterns: [
      ...
      },
      {
        ...
        re: /^\s*#{1,6}\s*(?:Conclusion|Summary|Final [Tt]houghts|Key [Tt]akeaways?|Wrapping [Uu]p|The [Bb]ottom [Ll]ine|Future [Oo]utlook|Looking [Aa]head)\s*:?\s*$/gm,
        note: '',
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    patterns: [
      {
        id: 'emoji_heading',
        re: /^\s*#{1,6}\s*\p{Extended_Pictographic}/gmu,
        note: 'Markdown heading opening with an emoji.',
        examples: ['## 🚀 Getting Started']
      },
      {
        id: 'emoji_bullet',
        re: /^\s*(?:[-*+]|\d+\.)\s+\p{Extended_Pictographic}/gmu,
        note: 'List item opening with an emoji.',
        examples: ['- ✅ Ship the feature']
      },
      {
        id: 'emoji_label_line',
        re: /^\s*\p{Extended_Pictographic}[️‍\p{Extended_Pictographic}]*\s*\*{0,2}[A-Z][^\n]{0,60}:/gmu,
  ...
  /* ---------------------------------------------------------------- */
  {
    ...
    patterns: [
      ...
      },
      {
        ...
        re: /^\s*#{2,6}\s+(?:[A-Z][a-z']+\s+){2,}[A-Z][a-z']+\s*$/gm,
